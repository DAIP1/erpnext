...
def get_default_bom_item(item_code):
	...
	return bom
# Hàm dùng lấy dữ liệu khi tạo yêu cầu nguyên liệu thô
@frappe.whitelist()
def make_raw_material_request(items, company, sales_order, project=None):
	# kiểm tra quyền chỉnh sửa đơn,nếu không có thì báo lỗi.
	if not frappe.has_permission("Sales Order", "write"):
		frappe.throw(_("Not permitted"), frappe.PermissionError)
	# Truyền giá trị vào biến items được lấy dưới dạng Json
	if isinstance(items, str):
		items = frappe._dict(json.loads(items))
	# lấy thông tin vật liệu thôi cần để làm sản phẩm
...
@frappe.whitelist()
# hàm lấy dữ liệu đơn mua để chuyển nhựng sang công ty khác cung cấp để tiến hành giao dịch mua hàng
